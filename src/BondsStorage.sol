...
contract BondsStorage is AccessControlRoleGroup, AccessControlProxy {
    ...
    error BondsTypeNotExist(uint256 id);
    error BondsTypeExist(uint256 id);
    ...
    function addBondsType(uint256 typeID, string memory nameType, string memory descriptionType, string memory feeType)
        external
        onlyRoleGroup(AccessControlRoleGroup.STN_GROUP, AccessControlRoleGroup.CREATE_TYPE_ROLE)
    {
    ...
    function getBondsType(uint256 typeId) external view returns (uint256, string memory, string memory, uint256) {
        if (!existsBondType(typeId)) {
        ...
        return (resultBond.TypeID, resultBond.Name, resultBond.Description, resultBond.MaxSupply);
    }
    ...
    function addBondsTypeMetadata(uint256 typeId, uint256[] memory metadataIds, Metadata[] memory metadatas)
        external
        onlyRoleGroup(AccessControlRoleGroup.STN_GROUP, AccessControlRoleGroup.CREATE_TYPE_ROLE)
    {
        if (!existsBondType(typeId)) {
    ...
    function getIdxBondsTypeMetadata(uint256 typeId) external view returns (uint256[] memory) {
        if (!existsBondType(typeId)) {
            revert BondsTypeNotExist(typeId);
        }
    ...
    function getBondsTypeMetadata(uint256 typeId, uint256 idMetadata) public view returns (Metadata memory) {
        if (!existsBondType(typeId)) {
            revert BondsTypeNotExist(typeId);
        }
    ...
    function getBondsTypeMetadataRoleAcess(uint256 typeId, uint256 idMetadata) public view returns (bytes32) {
        if (!existsBondType(typeId)) {
            revert BondsTypeNotExist(typeId);
        }
    ...
    function createTreasuryBonds(Transaction calldata transaction)
        external
        onlyRoleGroup(AccessControlRoleGroup.STN_GROUP, AccessControlRoleGroup.CREATE_BOND_ROLE)
    {
    ...
    function createTreasuryBondsValues(uint256 bondID, uint256[] calldata metadataIds, Values[] calldata values)
        external
        onlyRole(AccessControlRoleGroup.CREATE_BOND_VALUES)
    {
        ...
        for (uint256 i; i < metadataIds.length; i++) {
            role = getBondsTypeMetadataRoleAcess(treasuryBonds.TypeID, metadataIds[i]);
            accessControl.verifyRole(role, msg.sender);
    ...
    function getTreasuryBondsValues(uint256 bondID) external view returns (TreasuryBondsValue[] memory) {
        if (!existsTreasuryBonds(bondID)) {
            revert TreasuryBondsNotExist(bondID);
        }
        ...
        uint256[] memory idxMetadatas = mapBondTypes[treasuryBonds.TypeID].idxMetadatas;
        TreasuryBondsValue[] memory tBondsValues = new TreasuryBondsValue[](idxMetadatas.length);

        for (uint256 i = 0; i < idxMetadatas.length; i++) {
            ...
            tBondsValues[i].Value = getStringValue(metadata._Type, treasuryBonds.Values[idxMetadatas[i]]);
        }

        return tBondsValues;
    }

    function constructTokenURI(uint256 bondID) public view returns (string memory) {
        string memory name = "";
        ...
        name = string(
            abi.encodePacked(Strings.toString(treasuryBonds.Id), " - ", treasuryBonds.Code, "-", treasuryBonds.Name)
        );

        (uint256 year, uint256 month, uint256 day) = TimestampToDateYMD(treasuryBonds.MaturityDate);

        string memory date =
            string(abi.encodePacked(Strings.toString(day), "/", Strings.toString(month), "/", Strings.toString(year)));
        ...
        image = NFTRenderer.render(
            NFTRenderer.RenderParams({
                ...
                nameBonds: string(abi.encodePacked(treasuryBonds.Name, " | ", typeBond.Name, "-", typeBond.Description))
            })
        );
        ...
        for (uint256 i = 0; i < idxMeta.length; i++) {
    ...
    function existsTreasuryBonds(uint256 id) public view virtual returns (bool) {
