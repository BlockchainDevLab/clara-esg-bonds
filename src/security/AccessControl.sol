...
contract AccessControl {
    ...
    function setAuthorizationControl(AuthorizationControl authorizationControl_) external onlyMaster {
    ...
    function verifyRole(bytes32 role, address caller) public view returns (bool) {
        return authorizationControl.requireRole(role, caller);
    }

    function verifyRoleGroup(bytes32 group, bytes32 role, address caller) public view returns (bool) {
    ...
    function verifyAuthorizationControl(address authControl) internal view {
        ...
        bool checkIsAuthorizationControl =
            ERC165Checker.supportsInterface(authControl, type(IAuthorizationControl).interfaceId);
        require(checkIsAuthorizationControl, "AuthorizationControl address must be the same type IAuthorizationControl");
    }
}
