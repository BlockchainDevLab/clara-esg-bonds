...
contract AuthorizationControl is ERC165, IAuthorizationControl, Master {
    ...
    event RoleGroupGranted(bytes32 indexed group, bytes32 indexed role, address indexed addr);
    event RoleGroupRevoked(bytes32 indexed group, bytes32 indexed role, address indexed addr);
    ...
    error ROLES_AddressAlreadyHasRoleGroup(address addr, bytes32 group, bytes32 role);

    error ROLES_AddressDoesNotHaveRoleGroup(address addr, bytes32 group, bytes32 role);
    ...
    //Role Group
    mapping(address => mapping(bytes32 => mapping(bytes32 => bool))) public hasRoleGroup;
    ...
    function saveRoleGroup(bytes32 group, bytes32 role, address addr) public onlyMaster {
    ...
    function removeRole(bytes32 role, address addr) external onlyMaster {
        ...
        int256 idx = findIndex(addr, users);
        if (idx >= 0) {
    ...
    function removeRoleGroup(bytes32 group, bytes32 role, address addr) external onlyMaster {
        if (!hasRoleGroup[addr][group][role]) {
        ...
        int256 idx = findIndex(addr, usersGroup);
        if (idx >= 0) {
            uint256 i = uint256(idx);
    ...
    function requireRole(bytes32 role, address caller) public view returns (bool) {
        return hasRole[caller][role];
    }

    function requireRoleGroup(bytes32 group, bytes32 role, address caller) public view returns (bool) {
    ...
    function findIndex(address value, address[] memory values) public pure returns (int256) {
    ...
    function getUsersByRoleGroup(bytes32 group, bytes32 role) public view returns (address[] memory) {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IAuthorizationControl).interfaceId || super.supportsInterface(interfaceId);
    }
}
